...
function mergeArguments(args1, args2, config) {
    var result = deduplicateArguments(tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(args2), false), tslib.__read(args1), false).filter(utils.isSome));
...
function mergeArguments$1(a1, a2) {
    var e_1, _a;
    var result = tslib.__spreadArray([], tslib.__read(a2), false);
...
function mergeDirectives(d1, d2, config) {
    ...
    var result = deduplicateDirectives(tslib.__spreadArray([], tslib.__read(asNext), false));
...
function mergeDirective(node, existingNode) {
    if (existingNode) {
        ...
        return tslib.__assign(tslib.__assign({}, node), { locations: tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(existingNode.locations), false), tslib.__read(node.locations.filter(function (name) { return !nameAlreadyExists(name, existingNode.locations); })), false) });
    }
    return node;
}
...
function mergeEnumValues(first, second, config) {
    ...
    if (config === null || config === void 0 ? void 0 : config.consistentEnumMerge) {
        var reversed = [];
        if (first) {
            reversed.push.apply(reversed, tslib.__spreadArray([], tslib.__read(first), false));
        }
        first = second;
        second = reversed;
    }
    ...
    var result = tslib.__spreadArray([], tslib.__read(enumValueMap.values()), false);
    if (config && config.sort) {
        result.sort(utils.compareNodes);
    }
    return result;
...
function mergeFields(type, f1, f2, config) {
    ...
    var result = [];
    if (f2 != null) {
        result.push.apply(result, tslib.__spreadArray([], tslib.__read(f2), false));
    }
    if (f1 != null) {
...
function mergeNamedTypeArray(first, second, config) {
    ...
    var result = tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(second), false), tslib.__read(first.filter(function (d) { return !alreadyExists(second, d); })), false);
    if (config && config.sort) {
        result.sort(utils.compareNodes);
    }
    return result;
...
function mergeSchemaDefs(node, existingNode, config) {
    ...
    return ((config === null || config === void 0 ? void 0 : config.convertExtensions)
        ? tslib.__assign(tslib.__assign({}, node), { kind: graphql.Kind.SCHEMA_DEFINITION }) : node);
}
